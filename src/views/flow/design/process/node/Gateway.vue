...
<template>
  <div class="w-p-gateway">
    <div class="w-p-branch">
      <el-button v-if="!readonly" class="w-p-gateway-add" round plain @click="addBranch">
        添加分支
      </el-button>
      <!-- 分支遍历 -->
      <div
        v-for="(nodes, i) in modelValue.branch" :key="`${i}_br`"
        class="w-p-branch-item" :class="{ 'w-p-branch-item-l': i === 0,
                                          'w-p-branch-item-r': i === modelValue.branch.length - 1 }"
      >
        <div>
          <!-- 构造分支头节点 -->
          <component
            :is="NodeComponents[modelValue.props.type]" ref="node" v-model="modelValue.props.branch[i]" :readonly="readonly"
            :index="i"
            :move-rn="i < modelValue.branch.length - (modelValue.props.type !== 'Parallel' ? 2 : 1)" :move-ln="i > 0"
            :is-default="i === modelValue.branch.length - 1 && modelValue.props.type !== 'Parallel'"
          ...
          />
          <!-- 渲染分支 -->
          <template v-for="(nodei, bi) in nodes" :key="`${bi}_bi`">
            <!-- 分支里面继续遍历 -->
            <component
              :is="NodeComponents[nodei.type]" ref="node" v-model="modelValue.branch[i][bi]"
              :readonly="readonly" :branch="nodes" :index="bi"
              @insert-node="insertNode" @delete="deleteNode(nodes, bi)"
    ...
    <Node
      :readonly="readonly" :class="{ 'w-p-branch-end': branch.length === index + 1 }" :show-body="false"
