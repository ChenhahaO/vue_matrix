...
<template>
  <div class="w-p-node">
    ...
    <div
      v-if="showBody" class="w-p-node-body" :class="{ 'w-node-err': showError }"
      :style="{ '---shadowColor': headerColor }"
    >
      <div class="w-p-node-header" :style="{ 'background-color': headerColor }">
        <div>
          <el-icon v-if="!enableEdit" size="15">
            <component :is="headerIcon" />
          </el-icon>
          <input v-if="enableEdit" ref="nodeNameInput" v-model="_value.name" autofocus @blur="enableEdit = false">
          <div v-else class="w-row-text" @click="() => { if (!readonly) enableEdit = true }">
            {{ _value.name }}
          </div>
        </div>
        <el-icon v-if="showClose && !readonly" class="w-node-del" @click="emit('delete')">
          <Close />
        </el-icon>
      </div>
    ...
    <div class="w-p-node-add">
      <InsertBtn v-if="!readonly" @insert-node="type => emit('insertNode', type)" />
    </div>
  </div>
</template>
