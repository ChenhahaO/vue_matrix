...
<template>
  <div class="w-p-node">
    ...
    <div class="w-p-node-body" :class="{ 'w-node-err': showError }" :style="{ '---shadowColor': color }">
      ...
      <div style="flex: 1">
        <div class="w-p-node-header">
          ...
          <div v-if="!readonly && !isDefault" class="w-node-action">
    ...
    <div class="w-p-node-add">
      <InsertBtn v-if="!readonly" @insert-node="nd => emit('insertNode', nd)" />
    </div>
  </div>
</template>
