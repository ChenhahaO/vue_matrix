...
<template>
  <el-form>
    <el-form-item v-if="_value.props.groups.length > 1" label="条件组关系">
      <el-radio-group v-model="_value.props.logic">
        <el-radio label="全部满足" :value="true" />
        <el-radio label="满足任意一个" :value="false" />
      </el-radio-group>
    </el-form-item>
    <el-button style="margin-bottom: 20px;" link type="primary" icon="Plus" @click="addGroup">
      添加条件组
    </el-button>
    <ConditionGroupConfig
      v-for="(group, i) in _value.props.groups" :key="i" v-model="_value.props.groups[i]"
      :name="`条件组 ${i + 1}`" @delete="delGroup(i)"
    />
  </el-form>
</template>
