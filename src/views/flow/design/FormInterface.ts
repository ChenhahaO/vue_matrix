 ...
 * @param json 表单的json
 */
export function formInit(ref, json) {
  if (ref) {
 ...
 */
function loadFormItem(item, items, preName, addItemFunc) {
 ...
 * @returns {*[]} 表单组件列表
 */
export function resolveFormJson(json) {
  const items = []
  formItemMap.clear()
 ...
 * @returns [{表单字段信息}]
 */
export function getFormPermFields(defaultPerm = 'R') {
  const items = []
